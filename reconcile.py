...
def match_files_to_payments(csv_files: dict, payment_rows: list):
    """
    Match CSV files to payment IDs by comparing totals.
    Returns: matched {payment_id: (filename, date)}, unmatched_files, unmatched_payments
    """
    payment_totals = {
        row['payment_id']: (
...
def build_part1_df(csv_files: dict, matched: dict, invoice_bytes: bytes) -> pd.DataFrame:
    """Combine matched CSVs and add InInvoice flag from TicketVault invoice details."""
    inv_df  = pd.read_excel(io.BytesIO(invoice_bytes))
    inv_ids = set(inv_df['Ext Order #'].dropna().astype(int))
    ...
    combined['InInvoice'] = combined['TransactionID'].apply(
        lambda x: 1 if int(x) in inv_ids else 0
    )
    return combined
...
def build_part2_data(inv_det_bytes: bytes, inv_bytes: bytes):
    """
    Returns (date_range, poc_rows, poc_total, pod_data) ready for writing.
    """
...
def write_combined_xlsx(part1_df: pd.DataFrame, part2_data: tuple) -> bytes:
    """
    Write a single xlsx with:
      - Sheet 1: 'StubHub Details'  (Part 1 data)
      - Sheet 2: 'Network Summary'  (Part 2 summary)
    Returns bytes.
    """
...
def _write_part1_sheet(ws, df: pd.DataFrame):
    ...
    for col_idx, col_name in enumerate(hdr_cols, 1):
